...
- 금액은 **회차별 형태에서 자동 산정**한다(`scripts/fee_rules.py`). 사람에게 금액을 묻지 않는다.
  물어야 할 것은 **각 회의가 대면이었는지 비대면이었는지**다.
- 월 상한은 같은 전문가의 **기존 청구 건까지 합산**해 검사한다(`claims/*/claim.json`).
  초과하면 상한까지만 청구하고, 이월 여부를 사용자에게 묻는다.
- 비대면 건수가 홀수면 남는 1건이 200,000원으로 계산된다. 다음 청구와 묶을지 확인한다.
- 단가를 바꾸려면 `~/.claude/snu-expert-fee/config/fee_rules.json` 을 만든다.

## 0-2. 서명·날인·동의 (사람 손이 필요 없다)

| 항목 | 처리 |
|---|---|
| 지급받는자 전자서명 | 양식 `B17` 의 서명 이미지를 **재활용**. zip 바이트 복사라 그대로 보존된다 |
| 개인정보 동의 3항목 | 양식에 **이미 체크되어 있다**(Check Box 7·8·9). P7 에서 상태만 검사한다 |
| 연구책임자 날인 | **생략**(2026-08 확정) |

셋 다 잔여 작업 목록에 넣지 않는다. 다만 P7 검사에서 서명 이미지가 사라졌거나 체크가
3개 미만이면 실패로 보고한다.
...
1. **회차별 대면/비대면** — 최우선. 단가가 여기서 갈린다(대면 40만 / 비대면 20만).
   회차가 여러 개면 **각각** 확인한다. 하나라도 미확인이면 금액을 확정할 수 없다.
2. **참석자** — 기본값 `박준오 연구원`, `신재솔 연구원`. 기본값을 그대로 제시하고 추가/제외만 물어본다.
3. **자문위원(전문가)** — 지급 대상. 기본값 `신재솔 (위데이터랩 대표)`.
4. **일자 / 회차** — 여러 회차면 회차별 일자·시간을 각각.
5. **활용시간(총 시간)** — 사용내역서 `시간/회당/장` 칸에 들어가는 숫자.
6. **장소** — 대면 기본 `서울대학교 220동 650호`, 비대면 기본 `온라인 Zoom 화상회의`.
   1번에서 형태가 정해지면 장소는 기본값으로 채우고 확인만 받는다.
7. **금액** — 묻지 않는다. 형태에서 자동 산정한 뒤 결과를 보여주고 이의만 받는다.
   월 상한 초과·비대면 홀수 같은 경고가 나오면 그때만 판단을 요청한다.
8. **제목 / 목적** — 사전 예측 초안을 제시하고 문구 수정만 받는다.
9. **첨부 구성** — 회의자료 예시, 회의 사진, 시스템 지도 포함 여부.
